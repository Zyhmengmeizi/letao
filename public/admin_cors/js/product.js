$(function(){
	...
	function GetProduct(){
		...
		$.ajax({
			type:'get',
			url:app.api + '/product/queryProductDetailList',
			data:{
				page:page,
				pageSize:pagesize
			},
			success:function(result){
				...
				$('#productBox').html(template('productTpl',{data:result,api:app.api}));
	...
	$.ajax({
		url:app.api + '/category/querySecondCategoryPaging',
		type:'get',
		data:{
			page:1,
			pageSize:100
	...
	$('#fileUpload').fileupload({
	    dataType: 'json',
	    	...
	    	$('#imgBox').html(template('imgsTpl',{data:pic,api:app.api}))
	...
	$('#addProduct').on('click',function(){
		...
		$.ajax({
			type:'post',
			url:app.api + '/product/addProduct',
			data:{
				proName:productName,
